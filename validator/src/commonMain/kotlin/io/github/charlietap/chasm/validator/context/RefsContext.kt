...
internal class RefsContextImpl : RefsContext {
    ...
    internal fun reset(module: Module) {
        ...
        functionReferences += references(module)
    }
    ...
    private fun exportFunctionReferencesIndices(module: Module) = module.exports.mapNotNullTo(mutableSetOf()) { export ->
    ...
    private fun globalFunctionReferences(module: Module) = module.globals.flatMap { global ->
        global.initExpression.instructions.filterIsInstance<ReferenceInstruction.RefFunc>()
    }

    private fun tableFunctionReferences(module: Module) = module.tables.flatMap { table ->
        table.initExpression.instructions.filterIsInstance<ReferenceInstruction.RefFunc>()
    }

    private fun dataSegmentFunctionReferences(module: Module) = module.dataSegments.flatMap { dataSegment ->
    ...
    private fun elementSegmentFunctionReferences(module: Module) = module.elementSegments.flatMap { elementSegment ->
    ...
    private fun references(module: Module): Set<Index.FunctionIndex> {
        val references = globalFunctionReferences(module) +
            tableFunctionReferences(module) +
            dataSegmentFunctionReferences(module) +
            elementSegmentFunctionReferences(module)
        val indices = references.mapTo(mutableSetOf(), ReferenceInstruction.RefFunc::funcIdx)

        return indices + exportFunctionReferencesIndices(module)
    }
}
